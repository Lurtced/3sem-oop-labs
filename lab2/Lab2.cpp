...
int main()
{
	...
	/*
	if (this == &v) проверяет, не пытается ли объект самоприсвоиться.
	После этой строки присутствует return *this;, который возвращает ссылку на текущий объект.
	*/
	...
	/*
	In my case, the if-statement is false so it is passed and the operation
	= works. In a=a case, if-statement is true so it steps into and just
	returns the address of the current object without changing any values.
	*/
	...
	Vector c = a;
	/*
	It calls copy constructor created automatically.
	*/
	...
	MyString ss = s;
	cout << ss.GetString() << " (copied successfully)" << endl;
	/*
	Нет, эта операция не вызывается в этом фрагменте, потому что здесь
	вызывается конструктор копирования, созданный автоматически.
	*/
	...
	Vector e = a + b; // copy constructor is called indeed
	...
	/*
	I chose to make a method of the class because it is more intuitive and
	does not require a friend declaration.
	*/
	...
	Vector v1(1, 1), v2(101, 101); // calls Vector(double, doble) twice

	Shape::PrintCount(); // expected 0 vectors

	Rect r(v1, v2); // calls Rect(Vector&, Vector&), then copy constructor twice

	Shape::PrintCount(); // expected 1 vector

	cout << "\n\n\tTest Rect\n";
	r.Out();

	r = Rect(10, 10, 50, 30); // calls Rect constructor, then Vector constructors for 
	//params, then Rect::operator=

	r.Out();

	r.Inflate(); // d = 1 (default), then creates Vector constructors, then sums, 
	//then reassigns

	//further is all the same
	cout << "\n\n\tTest Inflate\n";
	r.Out();
	...
	Shape::PrintCount();	// Вызываем без помощи объекта
	...
	delete pv;
	Shape::PrintCount();	// Опять без помощи объекта
	...
	cout << "\n\n\tTest abstract Shapes\n";
	{
		...
		Shape::PrintCount(); // +11 because Rect contains 2 Vectors
		// and Circle contains 1 Vector
		for (int i = 0; i < Shape::GetCount(); i++)
		{ // some objects seem to move twice, but it is not so.
			Shape* p = Shape::shapes[i]; // the thing is that
			p->Move(v); // this loop goes through ALL the Shapes,
			p->Out(); // including Shapes in Shapes (like Vector in
			cout << ";  Area = " << p->Area() << endl; // Circle),
		} // and it won't happen like that if we move just one shape,
		Shape::PrintCount(); // like if we move Circle, we move the
		delete pr; // Vector (center) as well, but we don't do
		Shape::PrintCount(); // it twice, we just do it simultaneously
		delete pc; // delete pr deletes Rect and 2 Vectors,
		Shape::PrintCount(); // delete pc deletes Circle and Vector
	}
	Shape::PrintCount();
	...
	/*
	It can be done this way only if we use Vector* or Vector&, but if we
	use Vector (as an object), we cannot just declare it, we must provide
	all its structure, because it means creating an object of class Vector
	and this operation requires constructors etc.
	*/
